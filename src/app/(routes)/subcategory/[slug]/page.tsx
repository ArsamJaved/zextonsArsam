...
async function getSubCategoryData(subCategoryName: string) {
  try {
    const res = await fetch(
      `https://api.zextons.co.uk/get/subcategory/somedetails/${encodeURIComponent(
        subCategoryName
      )}`,
      {
        next: { revalidate: 60 },
...
async function getProducts(subCategoryName: string) {
  try {
    const res = await fetch(
      `https://api.zextons.co.uk/get/product/by/subcategory/${encodeURIComponent(
        subCategoryName
      )}`,
      { next: { revalidate: 60 } }
    );
        ...
        <div className="relative mb-5">
          {categoryData.banner?.path ? (
            <Image
              ...
              src={`https://api.zextons.co.uk/${categoryData.banner.path}`}
